// Copyright Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

// IWYU pragma: private, include "PerkEffectBase.h"

#ifdef FMP_PerkEffectBase_generated_h
#error "PerkEffectBase.generated.h already included, missing '#pragma once' in PerkEffectBase.h"
#endif
#define FMP_PerkEffectBase_generated_h

#include "UObject/ObjectMacros.h"
#include "UObject/ScriptMacros.h"

PRAGMA_DISABLE_DEPRECATION_WARNINGS

class AActor;

// ********** Begin Class UPerkEffectBase **********************************************************
#define FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_15_RPC_WRAPPERS_NO_PURE_DECLS \
	virtual void ApplyPerkEffect_Implementation(AActor* TargetActor); \
	DECLARE_FUNCTION(execApplyPerkEffect);


#define FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_15_CALLBACK_WRAPPERS
FMP_API UClass* Z_Construct_UClass_UPerkEffectBase_NoRegister();

#define FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_15_INCLASS_NO_PURE_DECLS \
private: \
	static void StaticRegisterNativesUPerkEffectBase(); \
	friend struct Z_Construct_UClass_UPerkEffectBase_Statics; \
	static UClass* GetPrivateStaticClass(); \
	friend FMP_API UClass* Z_Construct_UClass_UPerkEffectBase_NoRegister(); \
public: \
	DECLARE_CLASS2(UPerkEffectBase, UObject, COMPILED_IN_FLAGS(CLASS_Abstract), CASTCLASS_None, TEXT("/Script/FMP"), Z_Construct_UClass_UPerkEffectBase_NoRegister) \
	DECLARE_SERIALIZER(UPerkEffectBase)


#define FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_15_ENHANCED_CONSTRUCTORS \
	/** Standard constructor, called after all reflected properties have been initialized */ \
	NO_API UPerkEffectBase(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get()); \
	/** Deleted move- and copy-constructors, should never be used */ \
	UPerkEffectBase(UPerkEffectBase&&) = delete; \
	UPerkEffectBase(const UPerkEffectBase&) = delete; \
	DECLARE_VTABLE_PTR_HELPER_CTOR(NO_API, UPerkEffectBase); \
	DEFINE_VTABLE_PTR_HELPER_CTOR_CALLER(UPerkEffectBase); \
	DEFINE_ABSTRACT_DEFAULT_OBJECT_INITIALIZER_CONSTRUCTOR_CALL(UPerkEffectBase) \
	NO_API virtual ~UPerkEffectBase();


#define FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_12_PROLOG
#define FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_15_GENERATED_BODY \
PRAGMA_DISABLE_DEPRECATION_WARNINGS \
public: \
	FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_15_RPC_WRAPPERS_NO_PURE_DECLS \
	FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_15_CALLBACK_WRAPPERS \
	FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_15_INCLASS_NO_PURE_DECLS \
	FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h_15_ENHANCED_CONSTRUCTORS \
private: \
PRAGMA_ENABLE_DEPRECATION_WARNINGS


class UPerkEffectBase;

// ********** End Class UPerkEffectBase ************************************************************

#undef CURRENT_FILE_ID
#define CURRENT_FILE_ID FID_Users_Siddg_Downloads_FMPGame_FMP_Source_FMP_Public_PerkEffectBase_h

PRAGMA_ENABLE_DEPRECATION_WARNINGS
